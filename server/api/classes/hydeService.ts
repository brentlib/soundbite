...
export async function generateHydeSnippet(query: string): Promise<string> {
    ...
    const instructions = `Write a single paragraph that could appear in a document that answers the user's query. Write it as a factual excerpt from a reference source - an encyclopedia entry, manual, report, or article - not as a reply to a user. 
    
    Rules:
    - Answer the question directly and confidently. Do not hedge, flag uncertainty, ask for clarification, or mention yourself.
    - One tight paragraph, roughly 150-250 tokens.
